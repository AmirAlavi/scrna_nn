...
get_distance_data <- function(cur_table, type_factor_vector, type) {
  
    # Fill in the distance dictionary
    dist_dict <- list(dict_1=list(), dict_2=list(), dict_3=list(), dict_4=list(), dict_5=list())
    for(o_type in levels(type_factor_vector)){
        if (o_type != type){
            cur_row <- subset(cur_table, cur_table[[2]]==o_type)
            dist <- cur_row[[3]]
            if (dist == 1){
                n <- length(dist_dict$dist_1)
                n <- n + 1
                dist_dict$dist_1[[n]] <- o_type
            }
            else if (dist == 2){
                n <- length(dist_dict$dist_2)
                n <- n + 1
                dist_dict$dist_2[[n]] <- o_type
            }
            else if (dist == 3){
                n <- length(dist_dict$dist_3)
                n <- n + 1
                dist_dict$dist_3[[n]] <- o_type
            }
            else if (dist == 4){
                n <- length(dist_dict$dist_4)
                n <- n + 1
                dist_dict$dist_4[[n]] <- o_type
            }
            else {
                n <- length(dist_dict$dist_5)
                n <- n + 1
                dist_dict$dist_5[[n]] <- o_type
            }
        }
    }
    ...
    # Solidify the distance list.
    index <- 1
    dist_list <- list()
    for (dict in dist_dict) {
        if (length(dict) > 0){
            dist_list[[index]] <- dict
            index <- index + 1
        }
    }
...
dist_table <- read.delim("./distance.txt", header = FALSE, sep = "\t")
...
for(type in levels(type_factor_vector)){
    ...
    ## Get the subset of distance table for current type, and find the distance groups
    cur_table <- subset(dist_table, dist_table[[1]]==type)
    dist_groups <- get_distance_data(cur_table, type_factor_vector, type)
    ...
    for(exp in uniq_cur_type_experiments){
        ...
        if (ncol(other_types_counts) > ncol(cur_type_counts)) {
            ...
            # a distance-based sampling method: 
            # sample about equal number of cells in each distance group; within a distance group, sample randomly
            num_groups <- length(dist_groups)
            ...
            if (num_groups==1){
                ...
                raw_num_samples_per_group <- as.integer(100/num_groups)
                rem <- 100 - raw_num_samples_per_group*num_groups
            
                other_types_counts <- c()
            
                for(group in dist_groups){
                    if (rem > 0){
                        # remaindar correction for sample size per group
                        num_samples_per_group <- raw_num_samples_per_group + 1
                        rem <- rem - 1
                    }
                    else{
                        num_samples_per_group <- raw_num_samples_per_group
                    }

                    print("Group:")
                    print(group)
                    group_selection_vector <- rep(FALSE, length(type_factor_vector))

                    for(o_type in group) {
                        o_type_selection_vector <- type_factor_vector == o_type

                        group_selection_vector <- group_selection_vector | o_type_selection_vector
                    }
                    group_counts = counts_mat[, group_selection_vector]
                        
                    num_group_sampled_cells <- min(ncol(group_counts), num_samples_per_group)
                    cat("Number of group sampled cells: ", num_group_sampled_cells, "\n")
                    
                    # randomly sample within current group
                    group_counts <- group_counts[, sample(ncol(group_counts), num_group_sampled_cells)]
                    
                    other_types_counts <- cbind(other_types_counts, group_counts)
        ...
        t0 <- proc.time()
        scde.fitted.model <- knn.error.models(counts=combined_counts, groups=groups, n.cores=n.cores, save.model.plots=F)
        print(proc.time() - t0)
